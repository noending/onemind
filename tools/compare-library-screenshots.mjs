...
const defaults = {
  design: path.join(root, "docs/ui-parity/screenshots/design/library.png"),
  mini: path.join(root, "docs/ui-parity/screenshots/miniapp/library.png"),
  diff: path.join(root, "docs/ui-parity/screenshots/diff/library-diff.png"),
  threshold: 0.1,
  maxDiffRatio: 0.01
};
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (!value && key !== "--auto-resize-mini") continue;
...
async function main() {
  ...
  if (img1.width !== img2.width || img1.height !== img2.height) {
    ...
    const resizedMiniPath = path.join(path.dirname(args.mini), "library.auto-resized.png");
  ...
  console.log("Library screenshot diff report");
